defmodule Diplomat.ValueTest do
  ...
  describe "Value.new/1" do
    test "given an Entity" do
      ...
      assert Value.new(entity) == %Value{value: entity, exclude_from_indexes: false}
    end

    test "given a Key" do
      key = Key.new("TestKind", "1")
      assert Value.new(key) == %Value{value: key, exclude_from_indexes: false}
    end

    test "given a Value" do
      value = Value.new(1)
      assert Value.new(value) == %Value{value: value, exclude_from_indexes: false}
    end

    test "given a struct" do
      struct = %TestStruct{foo: "bar"}
      entity = Entity.new(%{"foo" => "bar"})
      assert Value.new(struct) == %Value{value: entity, exclude_from_indexes: false}
    end

    test "given a map" do
      map = %{"foo" => "bar"}
      entity = Entity.new(map)
      assert Value.new(map) == %Value{value: entity, exclude_from_indexes: false}
    end
    ...
    test "given a string" do
      string = "test"
      assert Value.new(string) == %Value{value: string, exclude_from_indexes: false}
    end

    test "given an integer" do
      int = 1
      assert Value.new(int) == %Value{value: int, exclude_from_indexes: false}
    end

    test "given a DateTime" do
      time = DateTime.utc_now()
      assert Value.new(time) == %Value{value: time, exclude_from_indexes: false}
    end
  ...
  describe "Value.new/2" do
    ...
    test "given a deeply nested map and exclude_from_indexes contains a path" do
      ...
      deeply_nested_entity = %Entity{
        properties: %{"baz" => %Value{value: "qux", exclude_from_indexes: true}}
      }

      nested_entity = %Entity{
        properties: %{"bar" => %Value{value: deeply_nested_entity, exclude_from_indexes: false}}
      }
  ...
  describe "Value.proto/1" do
    ...
    test "given a DateTime value" do
      ...
      assert Value.proto(datetime) ==
               PbVal.new(
                 value_type: {:timestamp_value, pb_timestamp},
                 exclude_from_indexes: false
               )
    end
